 ...
 * @version $Id$
*/
public class CreateEndEntityWizard extends JDialog
{
	/**
	 * Commons logging log instance
	 */
	private static Log log = LogFactory.getLog(CreateEndEntityWizard.class);

	public CreateEndEntityWizard()
	{
		...
		wc.addWizardListener(new WizardListener()
		{
					...
					WizardSettings settings)
			{
				...
				CreateEndEntityWizard.this.dispose();
			}
			...
			public void onPageChanged(WizardPage newPage, List<WizardPage> path)
			{
				...
				*/
				...
				CreateEndEntityWizard.this.setTitle(newPage.getTitle());
			}
		});
		...
		// Set a preferred size so that the wizard's initial size is something reasonable (as opposed to the default when no preferred size is set).
	...
	private class SiO2TestFactory implements PageFactory
	{
		...
		private final WizardPage[] pages =
			{
					new SelectEndEntityProfilePage("Select End Entity Type",
							"Select the type of end entity to create"),
